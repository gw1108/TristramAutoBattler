...
## the Inn begins as a ruin. Walk up and press interact to spend the rebuild
## cost from the treasury (GameState.spend_gold) and restore the building.
## Shop inventory/UI come in later slices.
## A static prop with a base-footprint collider, like the Inn.

const BUILDING_ID := "weapon_shop"
...
func _ready() -> void:
	...
	circle.radius = BalanceData.get_value("weapon_shop_interact_radius", 72.0)
	shape.shape = circle
