...
## The Armor shop (armourer's workshop). Starts ruined per the GDD — every
...
func _ready() -> void:
	position = Vector2(
		BalanceData.get_value("armor_shop_town_pos_x", 848.0),
		BalanceData.get_value("armor_shop_town_pos_y", 712.0))
	...
	circle.radius = BalanceData.get_value("armor_shop_interact_radius", 72.0)
	shape.shape = circle
