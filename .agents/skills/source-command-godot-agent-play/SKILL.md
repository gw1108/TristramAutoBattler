---
name: "source-command-godot-agent-play"
...
# source-command-godot-agent-play

Use this skill when the user asks to run the migrated source command `godot_agent_play`.
...
control channel; screenshots feed Codex's vision for visual/feel review.
...
`.env` and verifies the Codex CLI (the harness bills to your subscription via `Codex -p` — no
API key), installs this skill
into `.Codex/commands/`, adds the Playwright MCP server to `.mcp.json`, and wires the `AgentBridge`
...
8. Verify it compiles: `godot --headless --path <game> --import` (per AGENTS.md). Fix any GDScript
...
```
Ensure the Codex CLI is installed and logged in (`Codex /login`) with your Pro/Max subscription — the harness bills game-play LLM calls to it via `Codex -p`. No `ANTHROPIC_API_KEY` is required.
...
freezes the game while Codex thinks (`time_scale=0`), reads state/events, screenshots on the
...
Use this to explore the live game yourself / in a Codex session.
...
- **Audio is the lowest-confidence personality.** A headless browser is silent and Codex has no
