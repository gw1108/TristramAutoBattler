...
- +2 max morale every 2 levels (on odd levels after 1) — the first draft's +1 did not keep pace with the deeper dungeons' morale drain, so levelling bought no resilience against panic (see "Tuning notes for playtesting")
...
Morale Dmg   round(N / 2)           round(N / 2)          round(N / 2 + 1)
```

All archetypes have **0% guard and 0% crit chance** — blocking and critical hits are adventurer-side mechanics.

Morale damage is **half a point per dungeon level**, not the full point the first draft used. A dungeon level fields roughly `N` enemies and each of them hits for `N`, so at a full point per level the morale drain on a party grows as `N²` while the morale pool it lands on grows only with adventurer level — measured, that made every dungeon past level 1 unclearable at *any* party level, with parties fleeing in the first room rather than being worn down over the level. Halving it leaves dungeon level 1 identical (`round(0.5)` = 1 grunt, `round(1.5)` = 2 boss — every level-1 anchor below is untouched) and flattens the curve to 1/1/2/2 for grunts and casters and 2/2/3/3 for bosses. See "Tuning notes for playtesting".
...
| Grunt    | 2       | 13 | 5       | 4     | 0     | 100     | 140      | 1          |
| Caster   | 2       | 8  | 4 (AOE) | 3     | 0     | 90      | 140      | 1          |
| Boss 2   | 2       | 40 | 7       | 4     | 2     | 120     | 150      | 2          |
| Grunt    | 3       | 20 | 7       | 4     | 0     | 140     | 180      | 2          |
| Caster   | 3       | 13 | 6 (AOE) | 3     | 0     | 130     | 180      | 2          |
| Boss 3   | 3       | 64 | 9       | 4     | 3     | 160     | 190      | 3          |
| Grunt    | 4       | 33 | 9       | 4     | 0     | 180     | 220      | 2          |
| Caster   | 4       | 20 | 8 (AOE) | 3     | 0     | 170     | 220      | 2          |
| Boss 4   | 4       | 102 | 11     | 4     | 4     | 200     | 230      | 3          |
...
- Panic tuning: the 0.5 threshold is the main dial. Raising it to 0.6 makes morale a constant background pressure; lowering it to 0.4 makes morale a rare emergency. **Measured caveat:** it is only the main dial while morale damage is survivable at all. Swept headlessly over the first draft's numbers, moving the threshold to 0.4 or even 0.35 changed the dungeon level 4 clear rate by *nothing* — a party there was so far below the threshold that where the threshold sat was irrelevant. Check the drain before reaching for the threshold.
- Morale attrition (measured, and the reason morale damage is now `N/2` and growth is `+2`): the first draft's `N` per hit and `+1` max morale per 2 levels made every dungeon past level 1 a **0% clear at every party level up to 60**, for most party comps. The failure was not the slow grind down a level that it looks like on paper — parties fled in the *first room* of dungeon levels 3 and 4, having never seen the second. The cause is a scaling mismatch: morale damage per round grows as `N²` (about `N` enemies each hitting for `N`) while the morale pool grows linearly in adventurer level and not at all in dungeon level, so no amount of levelling could outrun it. Morale regen is irrelevant to this and is a red herring — it applies only *between* fights, and the party never survived the first one.
- Party composition is a bigger lever than it looks, and worth watching in any difficulty sweep: damage output, not tankiness, is what clears the deep levels. A Captain/Mage/Rogue party clears dungeon 4 where a Knight/Cleric/Berserker party of the same level does not, because the Mage's AOE is what kills `N`-enemy rooms before they grind the party down. Sweeping a single comp will mis-measure the game.
- **Open, and the next thing to look at:** the deep levels are still gated far above their `2N-1` encounter gate and the GDD's 14-day target for beating level 4, on the HP/damage race alone — with morale damage disabled entirely, dungeon level 4 still needs roughly a level 45 party. Equipment grants are the prime suspect, being specified here but not yet read by combat.
