...
| Dungeon entrance | The gateway to the dive; the bell stands beside it. Theme: **abandoned mine**. | Timber-braced mine portal cut into a rocky outcrop, cart tracks running into pitch darkness, cold unlit interior. Weathered support beams; reads as long-abandoned, not industrious. | MVP | [ ] `SourceArt/tools/generate_mine_entrance.py` exists but its PNG is not in the project |
