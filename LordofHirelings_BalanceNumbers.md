...
Variant tweaks are relative to that dungeon level's archetype row above. Each level has two grunt variants; which one spawns in a grunt slot (including the boss's escort) is a 50/50 roll. Boss signatures are deliberately one simple flag each — no new resource systems. Undead and demons are flavor on the same math; all enemies remain immune to morale damage. The four dungeon levels are data-driven from this bestiary; the archetype formulas above are what generalize.
...
| Trait   | Chance | Effect                              |
|---------|--------|-------------------------------------|
| Gilded  | 12%    | ×2 gold. Visually glints with coins.|
...
A full dungeon level 1 clear is 14 enemies, worth **22 XP** and roughly **23 gold** to each surviving member (13 non-boss enemies at ~1 gold each × the 1.22 expected trait multiplier, plus ~7.5 from the boss). 22 XP puts a fresh level 1 adventurer at level 2 with 14/16 progress toward level 3; two full clears reach level 3, which is exactly the gate for dungeon level 2. This lines up with the GDD's 3-day target for level 1.
